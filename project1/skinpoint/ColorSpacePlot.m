function [ output_args ] = ColorSpacePlot( pixMat )
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here

plotScatter(pixMat(:,4),pixMat(:,5),pixMat(:,end),'R','G');
plotScatter(pixMat(:,4),pixMat(:,6),pixMat(:,end),'R','B');
plotScatter(pixMat(:,5),pixMat(:,6),pixMat(:,end),'G','B');

plotScatter(pixMat(:,7),pixMat(:,8),pixMat(:,end),'Y','I');
plotScatter(pixMat(:,7),pixMat(:,9),pixMat(:,end),'Y','Q');
plotScatter(pixMat(:,8),pixMat(:,9),pixMat(:,end),'I','Q');

plotScatter(pixMat(:,10),pixMat(:,11),pixMat(:,end),'H','S');
plotScatter(pixMat(:,10),pixMat(:,12),pixMat(:,end),'H','V');
plotScatter(pixMat(:,11),pixMat(:,12),pixMat(:,end),'S','V');
...
%figure;
% h = histfit(skinMat(:,12))
% set(h(1),'FaceColor',[.8 .8 1]);
% set(h(1),'EdgeColor',[.8 .8 1]);
% set(h(2),'color','b');
% figure;
% h = histfit(nonSkinMat(:,12) );
% 
% pdS = fitdist(skinMat(:,12),'Normal')
% pdN = fitdist(nonSkinMat(:,12),'Normal')
...
%plot RGB
plot9Figures(skinMat(:,4:6),nonSkinMat(:,4:6),['R';'G';'B'])
plot9Figures(skinMat(:,7:9),nonSkinMat(:,7:9),['Y';'I';'Q'])
plot9Figures(skinMat(:,10:12),nonSkinMat(:,10:12),['H';'S';'V'])
...
xlabel('Red(R) component') %
ylabel('Sample size')%

subplot(3,3,2)
[n,xout] = hist(dataSet(:,5),256);
bar(xout,n,'FaceColor',[0.0824    0.4392    0.6510],'EdgeColor',[0.0824    0.4392    0.6510],'LineWidth',0.001)
xlabel('Green(G) component') %
ylabel('Sample size')%

subplot(3,3,3)
[n,xout] = hist(dataSet(:,6),256);
bar(xout,n,'FaceColor',[0.0824    0.4392    0.6510],'EdgeColor',[0.0824    0.4392    0.6510],'LineWidth',0.001)
xlabel('Blue(B) component') %
ylabel('Sample size')%
